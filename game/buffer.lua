...
function _R.Buffer:ReadUInt()
	return tonumber(ffi.cast('unsigned int', (bit.lshift( self:ReadByte(), 24 ) + bit.lshift( self:ReadByte(), 16 ) + bit.lshift( self:ReadByte(), 8 ) + bit.lshift( self:ReadByte(), 0 ))))
end
function _R.Buffer:ReadBUInt()
	return tonumber(ffi.cast('unsigned int', (bit.lshift( self:ReadByte(), 0 ) + bit.lshift( self:ReadByte(), 8 ) + bit.lshift( self:ReadByte(), 16 ) + bit.lshift( self:ReadByte(), 24 ))))
end
...
function _R.Buffer:WriteBShort( short )
	self:WriteByte( bit.band(short,0xFF) )
	self:WriteByte( bit.band(bit.rshift(short,8),0xFF) )
end
...
function _R.Buffer:ReadBShort()
	return bit.lshift( self:ReadByte(), 0 ) + bit.lshift( self:ReadByte(), 8 )
end
